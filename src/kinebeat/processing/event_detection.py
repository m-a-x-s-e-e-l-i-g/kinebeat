...
def detect_musical_events(
    ...
    *,
    ...
    for index, stem_name in enumerate(ordered):
        ...
        timing_frames = librosa.onset.onset_backtrack(peak_frames, onset_envelope)
        times = librosa.frames_to_time(timing_frames, sr=sample_rate)
        strengths = _normalized_strengths(onset_envelope, peak_frames)
        if stem_name == "drums":
            kinds = _classify_drum_events(signal, sample_rate, peak_frames)
        else:
            kind = {
                "bass": EventKind.BASS,
...
def _classify_drum_events(
    ...
    for frame in onset_frames:
        ...
        if low_ratio >= 0.15 or centroid < 1100:
